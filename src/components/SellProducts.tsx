...
const SellProducts = () => {
  ...
  const onSubmit = (form: any, e: any) => {
    ...
    ); //this is for making path of Image File
      ...
      getDownloadURL(storageRef)
        .then((url) => {
          addDoc(collection(db, "products"), {
            // setSuccessMessage("Your Product-List is Update"),
            productTitle,
            productType,
            description,
            brand,
  ...
  const handleProductImage = (e: any) => {
    ...
    if (selectedFile) {
      if (selectedFile && types.includes(selectedFile.type)) {
        ...
        console.log("inside Image", productImage);
        setImageError("");
      } else {
        // setProductImage(null)
        setImageError(
          "Please Select valid Image FIle Format(.jpg, .jpeg, .png, .gif)"
        );
      }
    } else {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <h3>FIll below details to Add your Products</h3>
          {successMessage && (
            ...
            <Controller
              ...
              render={(props: any) => (
                <TextField
                  ...
                  variant="standard"
                  {...props}
                  // value={productImage}    // In React, an <input type="file" /> is always an uncontrolled component because its value can only be set by a user, and not programmatically. so we can not take it as value of any"
                  // label="Product Image"
                  style={{ width: "40vw" }}
